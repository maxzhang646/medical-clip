"""Evaluate BioMedCLIP on OpenI bidirectional retrieval."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out", default="stage3_biomedclip_baseline.md")
...
@torch.no_grad()
def encode_dataset(model, tokenizer, loader: DataLoader, device: torch.device,
                   context_length: int) -> tuple[torch.Tensor, torch.Tensor]:
    image_embeddings = []
    text_embeddings = []
    for batch in loader:
        images = batch["image"].to(device)
        text_tokens = tokenizer(batch["caption"], context_length=context_length).to(device)
...
def main() -> None:
    ...
    print(f"Loading BioMedCLIP model: {args.model_name}")
    model, preprocess = open_clip.create_model_from_pretrained(args.model_name, device=device)
    ...
    dataset = ImageTextDataset(split_dataset.samples, preprocess)
    loader = DataLoader(
        dataset,
    ...
    image_embeddings, text_embeddings = encode_dataset(
        model,
        tokenizer,
        loader,
        device,
